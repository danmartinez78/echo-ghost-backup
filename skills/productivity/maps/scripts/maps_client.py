...
def cmd_terrain(args):
    """Query land cover and terrain features around a point for SAR intel.

    Returns structured terrain data: dominant land cover, water features,
    shelter potential, transit access, and a scatter-risk summary.
    Uses the Overpass mirror fallback chain automatically.
    """
    lat = float(args.lat)
    lon = float(args.lon)
    radius = args.radius

    # SAR-relevant terrain query: land cover + infrastructure near the LKP
    query = (
        f"[out:json][timeout:25];\n"
        f"(\n"
        ...
        f'  way["landuse"="farm"](around:{radius},{lat},{lon});\n'
        f'  way["landuse"="residential"](around:{radius},{lat},{lon});\n'
        f'  way["landuse"="commercial"](around:{radius},{lat},{lon});\n'
        f'  way["landuse"="industrial"](around:{radius},{lat},{lon});\n'
        f'  way["landuse"="grass"](around:{radius},{lat},{lon});\n'
    ...
    for el in elements:
        ...
        # Land cover bucketing
        if "natural" in tags:
            land_cover[tags["natural"]] = land_cover.get(tags["natural"], 0) + 1
        if "landuse" in tags:
            land_cover[tags["landuse"]] = land_cover.get(tags["landuse"], 0) + 1

        # Water features — collect name + type for SAR reference
        if tags.get("natural") == "water" or tags.get("water"):
            water_features.append({
                "name": tags.get("name", "unnamed"),
                "type": tags.get("water", tags.get("natural", "water")),
                "lat": el_lat,
                "lon": el_lon,
            })

        # Shelter: wood, forest, grassland — potential hideouts for scared pets
        if tags.get("natural") in ("wood", "tree_row") or tags.get("landuse") in ("forest", "grass"):
            key = f"{el_lat},{el_lon}"
            if key not in shelter_candidates:
                shelter_candidates[key] = {
        ...
        if tags.get("highway") == "bus_stop":
            shelter_candidates[f"{el_lat},{el_lon}_bus"] = {
                "type": "bus_stop",
    ...
    # Dominant terrain = land cover category with highest count
    dominant_terrain = max(land_cover, key=land_cover.get) if land_cover else "unknown"

    # SAR-specific risk flags
    risk_flags = []
    if land_cover.get("water", 0) > 0:
        risk_flags.append("water_nearby")
    if land_cover.get("forest", 0) + land_cover.get("wood", 0) > 0:
        risk_flags.append("vegetation_cover")
    if land_cover.get("residential", 0) > 2:
        risk_flags.append("high_residential_density")
    if road_count > 10:
        risk_flags.append("heavy_road_network")
    if not has_transit and road_count < 3:
        risk_flags.append("low_human_traffic")
    ...
    result = {
        ...
        "land_cover": dict(sorted(land_cover.items(), key=lambda x: -x[1])),
        "water_features": water_features[:10],
        "shelter_candidates": list(shelter_candidates.values())[:10],
...
def build_parser():
    ...
    p_terrain = sub.add_parser(
        "terrain",
        help="SAR terrain analysis around a point.",
        description=(
            "Query land cover, water features, shelter candidates, and transit access\n"
            "around a lat/lon point for lost-pet search intelligence.\n"
            "Uses Overpass API with automatic mirror fallback.\n"
            "Example: maps_client.py terrain 40.0853 -88.2456 --radius 1000"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    p_terrain.add_argument("lat", help="Latitude (decimal degrees).")
    p_terrain.add_argument("lon", help="Longitude (decimal degrees).")
    p_terrain.add_argument(
        "--radius", "-r",
